...
当前界面版本：**v3.3**。部署后左侧底部应显示 `UI 版本：v3.3`；如果未显示，说明仍在运行旧文件。

v3.2 重构了科学图表绘制：补齐横纵坐标刻度和轴标题，消除 Canvas 拉伸变形；默认单物质为甲醇，默认混合物为甲醇–水；频谱和混合物首次显示优先选择接近 25 ℃的数据。

v3.3 增加图表悬停数据提示；混合物按组成基准、温度和数据源严格分组，摩尔分数、质量分数、体积分数及摩尔浓度不混合绘图；组分顺序相反时统一换算到搜索框中的第一个组分。
...
- 数据总览与数据查询已合并为一个页面；查询支持两个混合物组分。
- Cole–Cole 检验支持“全部数据集”和单一数据集切换。
- 混合物图中实验值显示为散点，并按温度生成拟合曲线。
- 混合物组成基准按原始记录区分摩尔分数、体积分数、质量分数及摩尔浓度，不强制混为摩尔分数。
