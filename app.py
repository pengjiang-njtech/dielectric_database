...
def query_data(search='', tmin=None, tmax=None, fmin=None, fmax=None, limit=5000):
    ...
    SELECT sample_type AS 类型,
           ...
           source_name AS 数据来源, source_detail AS 来源详情, doi AS DOI
...
st.title("介电性质数据库")
st.markdown(
    "<div class='title-sub'>有机溶剂与混合溶剂 · 静态介电常数 εs · 介电常数 ε′ · 介电损耗 ε″</div>",
    unsafe_allow_html=True
)
...
with tab0:
    ...
    st.caption("总览默认包含人工整理/推荐数据与清洗后的 literature-mined 数据，不显示 NPL 原始重复测量层。")
